...
void si4705_init() {
	...
	gpio_mode(GPIOA, RF_SCLK, GPIO_MODE_OUT_50 | GPIO_CFGO_A_PUSH_PULL);
	gpio_mode(GPIOA, RF_SDO, GPIO_MODE_OUT_50 | GPIO_CFGO_A_PUSH_PULL);
	SPI1->CR1 |= SPI_CR1_BR_2 | SPI_CR1_BR_1 | SPI_CR1_BR_0 | SPI_CR1_MSTR | SPI_CR1_BIDIMODE | SPI_CR1_SSM | SPI_CR1_SSI;
#else
	gpio_mode(GPIOA, RF_SCLK, GPIO_MODE_OUT_50 | GPIO_CFGO_PUSH_PULL);
	gpio_mode(GPIOA, RF_SDO, GPIO_MODE_OUT_50 | GPIO_CFGO_PUSH_PULL);
#endif // CONTROL_HARDWARE_SPI

	gpio_mode(GPIOA, RF_NSEN, GPIO_MODE_OUT_50 | GPIO_CFGO_PUSH_PULL);
